...
func TestAccSplitUser_BasicWithName(t *testing.T) {
	email := testAccConfig.GetUserEmailorSkip(t)
	emailSplit := strings.Split(email, "@")
	emailFormatted := fmt.Sprintf("%s+%s@%s", emailSplit[0], acctest.RandString(8), emailSplit[1])
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccCheckSplitUser_basicWithName(emailFormatted, name),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(
						"split_user.foobar", "email", emailFormatted),
					resource.TestCheckResourceAttr(
...
func testAccCheckSplitUser_basicWithName(email, name string) string {
	return fmt.Sprintf(`
resource "split_user" "foobar" {
	email = "%s"
	name = "%s"
